...
const char *e1 = "L'usuari ja existeix";
const char *e2 = "L'usuari no existeix";
...
void Cjt_usuaris::afegir_usuari(const Usuari& u) {
    ...
    if (it != llista_usuaris.end()) throw ExcepcioEvaluator(e1);
    else {
...
void Cjt_usuaris::eliminar_usuari(string nom) {
    ...
    if (it == llista_usuaris.end()) throw ExcepcioEvaluator(e2);
    else {
